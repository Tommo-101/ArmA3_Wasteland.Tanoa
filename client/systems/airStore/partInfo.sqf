...
_animPhase = 1 - abs (airshop_list_checkboxTextures find (_partList lbPicture _index));
_partList lbSetPicture [_index, airshop_list_checkboxTextures select _animPhase]; // invert checkbox texture

_dialog = ctrlParent _partList;
_itemlist = _dialog displayCtrl airshop_veh_list;
_itemData = call compile (_itemlist lbData lbCurSel airshop_veh_list); // [name, class, price, type, variant, ...]
...
if ({isArray (_animSrcCfg >> _x)} count ["forceAnimate","forceAnimate2"] > 0) then
{
	...
	{
		[_animStates, _forceAnimate select _i, airshop_list_checkboxTextures select (_forceAnimate select (_i+1))] call fn_setToPairs;
	};
