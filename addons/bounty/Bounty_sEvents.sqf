  ...
  switch (toLower _type) do
  {
    case "addbounty":
    {
      ...
      if (_cost > 0) then
      {
        ...
        if (_bounty + _rewardAmount > ["A3W_bountyMax", 1000000] call getPublicVar) exitWith {}; // recipient would exceed or has reached max balance
    ...
    case "rewardbounty":
    {
      ...
      if (_reward > 0) then
      {
        ...
        _hunterBalance = (_hunterBalance + _reward) min (["A3W_atmMaxBalance", 100000000] call getPublicVar); // hunter exceeds max atm
